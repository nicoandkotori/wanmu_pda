...
public class DepartmentSearchAdapter extends BaseAdapter implements Filterable
{
    ...
    private ArrayList<Department> mOriginalValues;
    public DepartmentSearchAdapter(Context context, List<Department> list)
    {
    ...
    public View getView(int position, View convertView, ViewGroup viewGroup)
    {
        ...
        Department iconInformation = list.get(position);
        if(convertView == null)
        {
            view = layoutInflater.inflate(R.layout.search_department_item, null);
            viewHolder = new ViewHolder();
            viewHolder.depCode = (TextView) view.findViewById(R.id.cdepcode);
            viewHolder.depName = (TextView) view.findViewById(R.id.cdepname);
        ...
        viewHolder.depCode.setText(iconInformation.getCdepcode());
        viewHolder.depName.setText(iconInformation.getCdepname());
    ...
    private class ArrayFilter extends Filter {
        ...
        protected FilterResults performFiltering(CharSequence prefix) {
            ...
            if (prefix == null || prefix.length() == 0) {
                ...
                ArrayList<Department> values;
                synchronized (mLock) {//同步复制一个原始备份数据
                    values = new ArrayList<>(mOriginalValues);
                }
                final int count = values.size();
                final ArrayList<Department> newValues = new ArrayList<>();

                for (int i = 0; i < count; i++) {
                    final Department value = values.get(i);//从List<User>中拿到User对象
//                    final String valueText = value.toString().toLowerCase();
                    final String valueText = value.getCdepcode().toString().toLowerCase();//User对象的name属性作为过滤的参数
                    final String valueText1 = value.getCdepname().toString().toLowerCase();//User对象的name属性作为过滤的参数
        ...
        protected void publishResults(CharSequence prefix, FilterResults results) {
            ...
            list = (List<Department>) results.values;//此时，Adapter数据源就是过滤后的Results
            if (results.count > 0) {
