...
import com.myapplication.dto.st.BoxMain;
...
public class LclBoxActivity extends FragmentActivity {
    ...
    private EditText etPackCode;
    private EditText etPackId;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_lcl_box);
        //标题
        titleView = (TitleView) findViewById(R.id.titleView);
        titleView.setAppTitle("拼箱");
        ...
        etPackCode = (EditText) findViewById(R.id.packCode);
        etPackId = (EditText) findViewById(R.id.packId);
        ...
        mbtnSave.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                try {
                    ...
                    if(mDatas==null)
                    {
                        AlertDialog.Builder builder = new AlertDialog.Builder(LclBoxActivity.this);
                    ...
                    if (CustomStringUtils.isBlank(etPackId.getText().toString())) {
                        AlertDialog.Builder builder = new AlertDialog.Builder(LclBoxActivity.this);
                        builder.setMessage("保存失败，箱码不存在");
                        builder.setPositiveButton("确定",null);
                        builder.show();
                        IsSave=0;
                        return;
                    ...
                    mapquery.put("packId", etPackId.getText().toString() );
                    mapquery.put("userName", StringUrl.GetUser() );
                    dialog.show();
                    OkHttpUtils.postAsyn(StringUrl.GetUrl() + "/api/st/boxmain/savelcl", mapquery, new HttpCallback() {
                        @Override
    ...
    private void  InitDates(List<BarcodeProduct> list)
    {
        ...
        try
        {
            commonAdapter = new CommonAdapter<BarcodeProduct>(this,list, R.layout.activity_lcl_box_item) {
    ...
    private void BarScan(String barcode)
    {
        ...
        else   if(barcode.startsWith("box-")) {
            Map<String, String> mapquery = new HashMap<String, String>();
            mapquery.put("barcode", barcode.replace("box-",""));
            OkHttpUtils.postAsyn(StringUrl.GetUrl() + "/api/st/boxmain/getbyid", mapquery, new HttpCallback() {
                @Override
                public void onSuccess(ResultDesc resultDesc) {
                    super.onSuccess(resultDesc);
                    try {
                        if (resultDesc.getsuccess()) {
                            if (resultDesc.getresult() != null && resultDesc.getresult() != "null") {
                                //获取后台返回的数据
                                BoxMain data = JSON.parseObject(resultDesc.getresult().toString(), BoxMain.class);
                                //主表数据解析
                                if (data != null) {
                                    etPackCode.setText(data.getPackCode());
                                    etPackId.setText(data.getId());
                                } else {
