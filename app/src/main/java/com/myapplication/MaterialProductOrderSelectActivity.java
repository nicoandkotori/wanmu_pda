...
import com.myapplication.dto.mo.MomOrder;
...
public class MaterialProductOrderSelectActivity extends FragmentActivity {
...
//    private Button mbtnAll;//全选
    ...
    private List<MomOrder> mDatas;  //列表数据
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_material_product_order_select);



        //标题
        titleView = (TitleView) findViewById(R.id.titleView);
        titleView.setAppTitle("生产订单查询");
        ...
        etStartDate.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                ...
                DatePickDialogUtil date=new DatePickDialogUtil(MaterialProductOrderSelectActivity.this, new DatePickerDialog.OnDateSetListener() {
                    @Override
...
//        mbtnAll=(Button)findViewById(R.id.btnAll);
        ...
        mbtnYes.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                {
                    MomOrder dispatchList=mDatas.get(sRow);
                    Intent intent=new Intent();
                    intent.putExtra("vouchId",dispatchList.getMoid().toString());
                    intent.putExtra("vouchCode",dispatchList.getMocode());

//                    intent.putExtra("depName",dispatchList.getDepName());
//                    intent.putExtra("depCode",dispatchList.getDepCode());
                    intent.putExtra("sourceType","订单");
...
//                }
                ...
                OkHttpUtils.postAsyn(StringUrl.GetUrl()+"/api/mo/momorder/getlist",mapquery,new HttpCallback() {
                    @Override
                    public void onSuccess(ResultDesc resultDesc) {
                        super.onSuccess(resultDesc);
                        String res=resultDesc.getresult().toString();
                        mDatas= JSON.parseArray(res,MomOrder.class);
    ...
    //初始化列表数据
    private void  InitDates(List<MomOrder> list)
    {

        try
        {
            commonAdapter = new CommonAdapter1<MomOrder>(this,list, R.layout.activity_material_product_order_select_item) {

                @Override
                protected void convertView(final int pisition,View item, MomOrder s) {

                    if(s!=null)
                    {

                        SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd");
                        ((TextView) CommonViewHolder.get(item, R.id.vouchDate)).setText( "日期:"+ formatter.format(s.getCreatedate()));

                        if(s.getMocode()!=null)
                        {
                            ((TextView) CommonViewHolder.get(item, R.id.vouchCode)).setText("单号:"+s.getMocode().toString());
                        }
                        else
                        {
